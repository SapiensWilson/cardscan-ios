        ...
        do {
            ...
            let preprocessResult = try await ImagePreprocessor.process(image) { status, pct in
                Task { @MainActor in
                    ...
                    appState.processingProgress = pct * 0.4  // pre-proc takes 0–40%
                }
            }
            ...
            let ocrResult = try await OCREngine.recognise(preprocessResult.image) { pct in
                Task { @MainActor in
                    appState.processingProgress = 0.4 + pct * 0.55  // OCR takes 40–95%
                }
            }
            appState.rawOCRText = ocrResult.fullText

            // Phase C: Parse contact fields
            ...
            await MainActor.run {
                appState.isProcessing = false
                appState.processingStatus = "OCR failed: \(error.localizedDescription)"
